...
@section('content')
    ...
    <div class="container flex flex-col md:flex-row justify-center mx-auto px-4 py-4">
        <form class="w-full max-w-lg" action="{{ route('article.update', $article->slug) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
			<div class="flex flex-wrap -mx-3 mb-6">
				<div class="w-full px-3 mb-6 md:mb-0">
					...
					<input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" 
                        id="title" name="title" type="text" placeholder="Title" value="{{ $article->title }}">
					@if ($errors->has('title'))
				...
				<div class="w-full px-3">
					...
					<textarea class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" 
						id="description" name="description" type="text" placeholder="Description" rows="10">{{ $article->description }}</textarea>
				...
				<div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
					...
					<div class="relative">
						<select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-2 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
							id="type" name="type">
							<option {{ $article->type == 'Text' ? 'selected' : '' }}>Text</option>
							<option {{ $article->type == 'Video' ? 'selected' : '' }}>Video</option>
						</select>
